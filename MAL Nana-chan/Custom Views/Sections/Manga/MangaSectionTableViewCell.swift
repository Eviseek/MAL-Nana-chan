...
class MangaSectionTableViewCell: UITableViewCell {
    ...
    var height = 0
    var itemSection: Section<Manga>? = nil
    ...
    @IBAction func seeAllButtonClicked(_ sender: UIButton) {
        if let seeAllVC = parentVC?.storyboard?.instantiateViewController(withIdentifier: "SeeAllViewController") as? SeeAllViewController<Manga> {
    ...
    func itemSelected(id: Int) {
        if let controller = parentVC?.storyboard?.instantiateViewController(withIdentifier: "MangaDetailViewController") as? MangaDetailViewController {
...
extension MangaSectionTableViewCell: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "MangaCollectionViewCell", for: indexPath) as? MangaCollectionViewCell {
            var item = itemSection?.response.data[indexPath.item].node
            cell.mangaTitleLabel.text = item?.title
            if let score = item?.score {
                cell.mangaScoreLabel.text = score.description
            }
            if let downloadedImg = URL(string: item?.mainPicture?.medium ?? "") {
                cell.mangaImageView.af.setImage(withURL: downloadedImg)
            }
            return cell
        }
...
extension MangaSectionTableViewCell: UICollectionViewDelegateFlowLayout, UICollectionViewDelegate {
